...
import { FileReference, GroupOption, FileOption, SourceTreeType } from ".";
...
var FILETYPE_BY_EXTENSION: { [key: string]: string } = {
    a: 'archive.ar',
    ...
    GROUP_BY_FILETYPE: { [key: string]: string } = {
...
function defaultExtension(fileRef: FileReference) {
...
function defaultEncoding(fileRef: FileReference) {
...
function detectGroup(fileRef: FileReference, opt: GroupOption): string {
...
function detectSourcetree(fileRef: FileReference): SourceTreeType {
    ...
    return sourcetree as SourceTreeType;
}

function defaultPath(fileRef: FileReference, filePath: string) {
    var filetype = fileRef.lastKnownFileType || fileRef.explicitFileType,
...
function defaultGroup(fileRef: FileReference) {
...
class PbxFile implements FileReference {
    ...
    constructor(filepath: string, opt: FileOption) {
        ...
        if (opt.embed && opt.sign) {
            ...
            (this.settings.ATTRIBUTES as string[]).push('CodeSignOnCopy');
        }
    }
}
